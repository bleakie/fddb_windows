...
void hungarian_init(hungarian_t* prob, double** r, size_t m, size_t n, int mode)
{
  ...
  prob->m = m;
  prob->n = n;
  prob->r = (double**)calloc(m, sizeof(double*));
  prob->q = (int**)calloc(m, sizeof(int*));
  ...
  {
	prob->r[i] = (double*)calloc(n, sizeof(double));
	prob->q[i] = (int*)calloc(n, sizeof(int));
...
void hungarian_make_cover(hungarian_t* prob)
{
  double* row_max; 
  double* col_max; 
  row_max = (double*)calloc(prob->m, sizeof(double));
  col_max = (double*)calloc(prob->n, sizeof(double));
//   assert(prob);
