...
my $imDir = "F:/Project/FaceDetect/image/originalPics/"; 
# where the folds are
my $fddbDir = "../FDDB-folds"; 
# where the detections are
my $detDir = "pyramidbox";#"MTCNN"; 
###########################
...
my $gpFile = "./pyramidbox/ContROC.p";
my $gpFile1 = "./pyramidbox/DiscROC.p";
# read all the folds into a single file for evaluation
my $detFile = $detDir;
#$detFile =~ s/\//_/g;
$detFile = $detFile."/output-FDDB-pyramidbox.txt";
...
makeGNUplotFile("ContROC.txt", $gpFile, $detDir, $detDir."ContROC.png");
#system("echo \"load '$gpFile'\" | $GNUPLOT");

makeGNUplotFile("DiscROC.txt", $gpFile1, $detDir, $detDir."DiscROC.png");
#system("echo \"load '$gpFile'\" | $GNUPLOT");
